#!/bin/bash
# End-to-end tests for the siyuan-docker-compose backup + restore flow.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-siyuan}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-siyuan-docker-compose.yml}"
...
: "${SIYUAN_BACKUPS_PATH:=/srv/siyuan/backups}"
: "${SIYUAN_DATA_BACKUP_NAME:=siyuan-data-backup}"
: "${SIYUAN_BACKUP_INTERVAL:=24h}"

BACKUPS_PATH="${SIYUAN_BACKUPS_PATH%/}"
DATA_PREFIX="${SIYUAN_DATA_BACKUP_NAME}"
INTERVAL="${SIYUAN_BACKUP_INTERVAL}"
...
APP_CONTAINER="$(docker compose -f "$DOCKER_COMPOSE_FILE" -p "$COMPOSE_PROJECT_NAME" ps -aq siyuan | head -n 1)"
...
test_restore_roundtrip() {
  ...
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./siyuan-restore-data.sh "$(basename "$set" | grep -oE '[0-9]{4}-[0-9]{2}-[0-9]{2}_[0-9]{2}-[0-9]{2}')" > /dev/null \
    || { fail "./siyuan-restore-data.sh failed"; return 1; }
...
echo "=== siyuan: backup/restore E2E tests ==="
