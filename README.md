...
[![OpenSSF Best Practices](https://www.bestpractices.dev/projects/14852/badge)](https://www.bestpractices.dev/projects/14852)
...
Every release from v1.6.4 on carries three files made on GitHub's runner with a short-lived identity and no stored key: `siyuan-docker-compose-<tag>.tar.gz`, a `git archive` of exactly the tree the tag points at; `siyuan-docker-compose-<tag>.tar.gz.sigstore.json`, a keyless [Sigstore](https://www.sigstore.dev/) signature over it; and `siyuan-docker-compose-<tag>.intoto.jsonl`, [SLSA](https://slsa.dev/) build provenance from the SLSA generator. To check them with nothing from this repository trusted:

```bash
cosign verify-blob siyuan-docker-compose-<tag>.tar.gz \
  --bundle siyuan-docker-compose-<tag>.tar.gz.sigstore.json \
  --certificate-identity-regexp '^https://github.com/heyvaldemar/siyuan-docker-compose/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com

slsa-verifier verify-artifact siyuan-docker-compose-<tag>.tar.gz \
  --provenance-path siyuan-docker-compose-<tag>.intoto.jsonl \
  --source-uri github.com/heyvaldemar/siyuan-docker-compose
```
