#!/usr/bin/env bash
# siyuan-restore-data.sh [timestamp]
#
# Replaces SiYuan's data with one backup set: the data archive and, for each
# SQLite database, the consistent copy the backups service took beside it.
#
#   ./siyuan-restore-data.sh                      list the sets and ask
#   ./siyuan-restore-data.sh YYYY-MM-DD_hh-mm     restore that one
#
...
#
# WHAT THE ARCHIVE LEAVES OUT ON PURPOSE IS KEPT. The loop excludes temp and .siyuan/history
# from the archive; those are set aside, the rest is cleared and unpacked, and
# they are put back.
#
# CI runs this exact file.
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than siyuan.
set -Eeuo pipefail

PROJECT="${COMPOSE_PROJECT_NAME:-siyuan}"
STOP_SERVICES="siyuan"                   # every service that writes what is restored
ROOT="/data"                            # where the archive unpacks
PARTS="."                          # what it holds, relative to ROOT
KEEP="temp .siyuan/history"                            # what it leaves out on purpose, relative to ROOT
...
DIR="$(env_of SIYUAN_BACKUPS_PATH)"; DATA_NAME="$(env_of SIYUAN_DATA_BACKUP_NAME)"
DB_NAME=""; [ -z "$DBS" ] || DB_NAME="$(env_of SIYUAN_BACKUP_NAME)"
...
if ! docker exec "$BKP" sh -c "set -eu
  ...
  echo "error: the restore failed part-way; the data may be incomplete. Restore another set before using SiYuan." >&2
  exit 1
fi
